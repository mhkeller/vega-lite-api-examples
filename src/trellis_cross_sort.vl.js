...
export default function chart() {
	return {
		...
		transform: [
			{ joinaggregate: [{ op: 'median', field: 'x', as: 'median_x_by_a' }], groupby: ['a'] },
			{ joinaggregate: [{ op: 'median', field: 'y', as: 'median_y_by_b' }], groupby: ['b'] }
		],
		facet: {
			column: { field: 'a', sort: { op: 'median', field: 'x' } },
			row: { field: 'b', sort: { op: 'median', field: 'y' } }
		...
		spec: {
			...
			layer: [
				...
				{ mark: { type: 'rule' }, encoding: { x: { aggregate: 'median', field: 'median_x_by_a' } } },
				{ mark: { type: 'rule' }, encoding: { y: { aggregate: 'median', field: 'median_y_by_b' } } }
			]
		}
	};
}
