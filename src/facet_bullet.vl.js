...
export default function chart() {
	return vl
		...
		.spec(
			vl.layer(
				vl.markBar({ color: '#eee' }).encode(vl.x().field('ranges[2]')),
				vl.markBar({ color: '#ddd' }).encode(vl.x().field('ranges[1]')),
				vl.markBar({ color: '#ccc' }).encode(vl.x().field('ranges[0]')),
				vl.markBar({ color: 'lightsteelblue', size: 10 }).encode(vl.x().field('measures[1]')),
				vl.markBar({ color: 'steelblue', size: 10 }).encode(vl.x().field('measures[0]')),
				vl.markTick({ color: 'black' }).encode(vl.x().field('markers[0]'))
			).encode(vl.x().type('quantitative').scale({ nice: false }).title(null))
		)
