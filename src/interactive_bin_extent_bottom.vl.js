import * as vl from 'vega-lite-api';

/**
 * Write a Node.JS function that uses the vega-lite-api library to
 * generate and return the vega-lite JSON spec below.
 */

export default function chart() {
	return vl
		.vconcat(
			vl.markBar()
				.width(480)
				.height(150)
				.encode(
					vl.x().field('time').bin({ maxbins: 30, extent: { param: 'brush' } }),
					vl.y().aggregate('count')
				),
			vl.markBar()
				.width(480)
				.height(50)
				.params({ name: 'brush', select: { type: 'interval', encodings: ['x'] } })
				.encode(
					vl.x().field('time').bin({ maxbins: 30 }),
					vl.y().aggregate('count').axis({ title: 'Count' })
				)
		)
		.data({ url: 'data/flights-5k.json', format: { parse: { date: 'date' } } })
		.transform(vl.calculate('hours(datum.date) + minutes(datum.date) / 60').as('time'))
		.toSpec();
}

/*
{
  "$schema": "https://vega.github.io/schema/vega-lite/v5.json",
  "data": {
    "url": "data/flights-5k.json",
    "format": {"parse": {"date": "date"}}
  },
  "transform": [{"calculate": "hours(datum.date) + minutes(datum.date) / 60", "as": "time"}],
  "vconcat": [{
    "width": 480,
    "height": 150,
    "mark": "bar",
    "encoding": {
      "x": {
        "field": "time",
        "bin": {"maxbins": 30, "extent": {"param": "brush"}}
      },
      "y": {"aggregate": "count"}
    }
  }, {
    "width": 480,
    "height": 50,
    "params": [{
      "name": "brush",
      "select": {"type": "interval", "encodings": ["x"]}
    }],
    "mark": "bar",
    "encoding": {
      "x": {"field": "time", "bin": {"maxbins": 30}},
      "y": {"aggregate": "count", "axis": {"title": "Count"}}
    }
  }]
}
*/
