...
export default function chart() {
	...
	return vl
		.layer(
			vl.markBar()
				.encode(vl.color().fieldQ('count').title('Number of fruit')),
			vl.markText({ align: 'right', xOffset: -4, aria: false })
				.encode(
					vl.text().fieldQ('count'),
					vl.color().value('black').condition({ test: { field: 'count', gt: 10 }, value: 'white' })
				)
		)
		...
		.data({ values: data })
		.encode(
			vl.y().fieldN('name').sort('-x').title(null),
			vl.x().fieldQ('count').title(null)
		)
		.toSpec();
}
